...
/** The default cache duration when creating a new Registry, in minutes  */
export const DEFAULT_REGISTRY_CACHE = 15;
...
export class SettingsService {
  ...
  public getSettings() {
    let upTodate: Observable<ApplicationSettings>;
    if (!this.settingsSubject.value) {
      upTodate = this.ensureSettingsUpToDate();
    } else {
      upTodate = this.settingsSubject.asObservable();
    }
  ...
  private migrateSettingsIfNeeded(settings: ApplicationSettings) {
    ...
    if (dockerHub.url !== expectedUrl) {
      ...
      for (const registry of settings.registries) {
        ...
        if (typeof registry.enableCaching === 'undefined') {
          registry.enableCaching = true;
          registry.cacheDurationInMinutes = DEFAULT_REGISTRY_CACHE;
        }
      }
